...
In Vision, the modelling of an transformer load may seem to be different from an individual transformer and load.
There is a minor difference in both the reactive power consumed and generated. 
This could also correspond to a minor voltage deviation in the results.
...
- The source bus in power-grid-model is mapped with a source impedance. `Sk"nom`, `R/X` and `Z0/Z1` are the attributes used in modelling source impedance. In Vision, these attributes are used only for short circuit calculations
