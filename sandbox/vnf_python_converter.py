...
def parse_vnf(file_path: str|Path) -> pd.DataFrame:
    with open(file_path, "r") as file:
        file = file.read()
        components = ["NODE", "LINE", "SOURCE", "LOAD"]
        component_dict = {}
...
def parse_line(vnf_txt: str) -> pd.DataFrame:
    ...
    for line in lines:
        ...
        if line.startswith('#LinePart'):
            # TODO check if a cable can have multiple line parts
...
def parse_load(vnf_txt: str) -> pd.DataFrame:
    ...
    # TODO: how to include "gelijktijdigheid"? See excel converter yaml -> can be in NODE
...
component_data = TabularData(NODE=component_dict["NODE"], LINE=component_dict["LINE"], SOURCE=component_dict["SOURCE"], LOAD=component_dict["LOAD"])
