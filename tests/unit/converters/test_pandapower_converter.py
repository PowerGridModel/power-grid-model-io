...
def test_parse_data(create_input_data_mock: MagicMock):
    ...
    assert len(converter.pp_inputdata) == 1 and "bus" in converter.pp_inputdata
...
def test_create_pgm_input_object__empty(
    ...
    # Arrange
    converter = PandaPowerConverter()
    converter.pp_inputdata[table] = pd.DataFrame()  # type: ignore
...
def test_create_pgm_input_nodes(mock_init_array: MagicMock, two_pp_objs: MockDf, converter):
    # Arrange
    converter.pp_inputdata["bus"] = two_pp_objs
...
def test_create_pgm_input_lines(mock_init_array: MagicMock, two_pp_objs, converter):
    # Arrange
    converter.pp_inputdata["line"] = two_pp_objs
...
def test_create_pgm_input_sources(pp_example_simple: Tuple[PandaPowerData, float], pgm_example_simple: SingleDataset):
    ...
    converter.pp_inputdata = pp_example_simple[0]
...
def test_get_winding_types__value_error():
    # Arrange
    converter = PandaPowerConverter()
    converter.pp_inputdata = {"trafo": pd.DataFrame([(1, "ADyn")], columns=["id", "vector_group"])}
...
def test_get_trafo3w_winding_types__value_error():
    # Arrange
    converter = PandaPowerConverter()
    converter.pp_inputdata = {"trafo3w": pd.DataFrame([(1, "ADyndrr")], columns=["id", "vector_group"])}
...
def test_get_trafo3w_switch_states(mock_get_individual_switch_states: MagicMock):
    ...
    # Act
    actual = converter.get_trafo3w_switch_states(converter.pp_inputdata["trafo3w"])
...
def test_get_pp_attr_attribute_doesnt_exist():
    # Arrange
    converter = PandaPowerConverter()
    converter.pp_inputdata = {"trafo3w": pd.DataFrame([[2, 31, 315]], columns=["index", "mv_bus", "lv_bus"])}
