...
def test_converter__id_lookup(converter: VisionExcelConverter):
    ...
    assert converter._id_lookup(name="node", key=tuple(row_1.tolist())) == 0
    assert converter._id_lookup(name="node", key=tuple(row_2.tolist())) == 1
    assert converter._id_lookup(name="node", key=tuple(row_3.tolist())) == 0
    assert converter._id_lookup(name="node", key=tuple(row_1.tolist())) == 0
