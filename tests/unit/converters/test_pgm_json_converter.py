...
def test_parse_data(converter: PgmJsonConverter, structured_input_data, structured_batch_data):
    with pytest.raises(PowerGridSerializationError, match=r"Map or an array expected."):
    ...
    assert len(pgm_data[CT.node]) == 2
    assert [1, 2] in pgm_data[CT.node][AT.id]
    assert [400.0, 400.0] in pgm_data[CT.node][AT.u_rated]
    ...
    assert (pgm_batch_data[CT.sym_load]["indptr"] == np.array([0, 1, 3])).all()
    assert (pgm_batch_data[CT.sym_load]["data"][AT.id] == [3, 3, 4]).all()
    assert (pgm_batch_data[CT.sym_load]["data"][AT.p_specified] == [1.0, 2.0, 3.0]).all()
...
def test_parse_component(converter: PgmJsonConverter, structured_input_data):
    ...
    assert [1, 2] in node_array[AT.id]
    assert [400.0, 400.0] in node_array[AT.u_rated]
    assert extra_info == {2: {"some_extra_info": 2.1}}

    node_with_wrong_attr_val = {AT.id: 3, AT.u_rated: "fault"}
...
def test_serialize_data(converter: PgmJsonConverter, pgm_input_data: SingleDataset, pgm_batch_data: BatchDataset):
    ...
    assert structured_single_data == {CT.node: [{AT.id: 1}, {AT.id: 2}]}
...
def test_serialize_dataset(converter: PgmJsonConverter, pgm_input_data: SingleDataset, pgm_batch_data: BatchDataset):
    with pytest.raises(ValueError, match="Invalid data format"):
        converter._serialize_dataset(data={CT.node: "attribute"}, extra_info=None)  # type: ignore
    with pytest.raises(ValueError, match="Invalid data format"):
    ...
    assert structured_data == {CT.node: [{AT.id: 1}, {AT.id: 2}]}
    ...
    assert structured_data_with_extra_info == {CT.node: [{AT.id: 1, "dummy": "data"}, {AT.id: 2}]}
