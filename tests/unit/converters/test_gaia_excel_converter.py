...
from power_grid_model_io.converters.gaia_excel_converter import DEFAULT_MAPPING_FILE, GaiaExcelConverter
...
def test_mapping_files_exist(language: str):
    gf = Path(str(DEFAULT_MAPPING_FILE).format(language=language))
    assert gf.exists()
