...
class CsvDirStore(BaseDataStore[TabularData]):
    ...
    def save(self, data: TabularData) -> None:
        ...
        for table_name, table_data in data.items():
            table_data_df = table_data if isinstance(table_data, pd.DataFrame) else pd.DataFrame(table_data)
            table_data_df.to_csv(path_or_buf=self._dir_path / f"{table_name}.csv", **self._csv_kwargs)
