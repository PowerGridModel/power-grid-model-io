...
class PandaPowerConverter(BaseConverter[PandaPowerData]):
    ...
    def _create_pgm_input_lines(self):
        ...
        pgm_lines["i_n"] = (self._get_pp_attr("line", "max_i_ka", expected_type='f8') * 1e3) * self._get_pp_attr("line", "df", expected_type='f8',  default=1) * parallel
        ...
        pgm_lines["tan0"] = (
            self._get_pp_attr("line", "g0_us_per_km", expected_type='f8', default=0) / c0_nf_per_km / (2 * np.pi * self.system_frequency * 1e-3)
        )
    ...
    def _create_pgm_input_sources(self):
        ...
        pgm_sources["u_ref_angle"] = self._get_pp_attr("ext_grid", "va_degree", expected_type='f8', default=0.0) * (np.pi / 180)
    ...
    def _create_pgm_input_sym_gens(self):
        ...
        pgm_sym_gens["q_specified"] = self._get_pp_attr("sgen", "q_mvar", expected_type='f8', default=0.0) * (1e6 * scaling)
    ...
    def _create_pgm_input_asym_gens(self):
        ...
        pgm_asym_gens["node"] = self._get_pgm_ids("bus", self._get_pp_attr("asymmetric_sgen", "bus", expected_type='i8'))
    ...
    def _create_pgm_input_sym_loads(self):
        ...
        const_i_multiplier = self._get_pp_attr("load", "const_i_percent", expected_type='f8', default=0) * scaling * (1e-2 * 1e6)
        const_z_multiplier = self._get_pp_attr("load", "const_z_percent", expected_type='f8', default=0) * scaling * (1e-2 * 1e6)
    ...
    def _create_pgm_input_asym_loads(self):
        ...
        pgm_asym_loads["node"] = self._get_pgm_ids("bus", self._get_pp_attr("asymmetric_load", "bus", expected_type='u4'))
    ...
    def _create_pgm_input_transformers(self):  # pylint: disable=too-many-statements, disable-msg=too-many-locals
        ...
        checks = {
            ...
            "si0_hv_partial": np.isnan(self._get_pp_attr("trafo", "si0_hv_partial", expected_type='f8', default=np.nan)).all(),
        }
        if not all(checks.values()):
            failed_checks = ", ".join([key for key, value in checks.items() if not value])
        ...
        pgm_transformers["from_node"] = self._get_pgm_ids("bus", self._get_pp_attr("trafo", "hv_bus", expected_type='u4'))
    ...
    def _create_pgm_input_three_winding_transformers(self):
        ...
        clocks_12 = np.round(self._get_pp_attr("trafo3w", "shift_mv_degree", expected_type='f8', default=0.0) / 30.0) % 12
        clocks_13 = np.round(self._get_pp_attr("trafo3w", "shift_lv_degree", expected_type='f8', default=0.0) / 30.0) % 12
        ...
        pgm_3wtransformers["node_1"] = self._get_pgm_ids("bus", self._get_pp_attr("trafo3w", "hv_bus", expected_type='u4'))
        pgm_3wtransformers["node_2"] = self._get_pgm_ids("bus", self._get_pp_attr("trafo3w", "mv_bus", expected_type='u4'))
        pgm_3wtransformers["node_3"] = self._get_pgm_ids("bus", self._get_pp_attr("trafo3w", "lv_bus", expected_type='u4'))
    ...
    def _create_pgm_input_wards(self):
        ...
        pgm_sym_loads_from_ward["q_specified"][:n_wards] = self._get_pp_attr("ward", "qs_mvar", expected_type='f8') * 1e6
        ...
        pgm_sym_loads_from_ward["q_specified"][-n_wards:] = self._get_pp_attr("ward", "qz_mvar", expected_type='f8') * 1e6
    ...
    def _create_pgm_input_motors(self):
        ...
        pgm_sym_loads_from_motor["node"] = self._get_pgm_ids("bus", self._get_pp_attr("motor", "bus", expected_type='i8'))
    ...
    def _get_pp_attr(self, table: str, attribute: str, expected_type : Union[str, dtype(object)], default: Optional[Union[float, bool, str]] = None) -> np.ndarray:
        """
